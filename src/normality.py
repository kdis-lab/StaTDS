...
def skewness_test(b2, n):
    ...
    :param b2: The squared sample skewness.
    :param n: The sample size.

    :return: A tuple containing the following:
        - z_b1: The z-score for the skewness test. A higher absolute value indicates greater skewness.
        - p_value: The p-value corresponding to the z-score. A small p-value (typically ≤ 0.05) suggests
                   that the distribution of the data is significantly skewed.

    Note: The skewness test is a hypothesis test that evaluates whether the skewness of the data differs
...
def kurtosis_test(b2, n):
    ...
    :param b2: The squared sample kurtosis.
    :param n: The sample size.

    :return: A tuple containing the following:
        - z_b2: The z-score for the kurtosis test. This score indicates the degree of peakedness in the distribution.
        - p_value: The p-value corresponding to the z-score. A small p-value (typically ≤ 0.05) suggests that the
                   distribution of the data has significant kurtosis, differing from a normal distribution.

    Note: The kurtosis test is used to determine if a dataset has heavier or lighter tails compared to a normal
...
def qq_plot(data: np.array):
    ...
    :param data: A numpy array of sample data to be analyzed.

    :return: A matplotlib figure object that contains the QQ plot.

    Note: In the QQ plot, the quantiles of the data are plotted against the quantiles of a theoretical distribution
...
def pp_plot(data: np.array):
    ...
    :return: A matplotlib figure object that contains the PP plot.

    Note: In a PP plot, the y-axis shows the empirical CDF of the data, and the x-axis displays the theoretical CDF of
